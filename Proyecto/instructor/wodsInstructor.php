<?php
include '../modulos/headerInstructor.php';
?>
<section class="bg-white" id="pagina_wods">
    <div class="container mt-5">
        <div class="row">
            <div class="wods col-md-6 col-lg-4 mb-4">
                <a href="thegirls.php" class="image-container">
                    <img src="../img/the_girls.jpg" class=" img-wods img-thumbnail" alt="...">
                    <div class="overlay-text">
                        <div class="titulo-seccion">
                            The girls
                        </div>
                    </div>
                </a>
            </div>
            <div class="wods col-md-6 col-lg-4 mb-4">
                <a href="theheroes.php" class="image-container">
                    <img src="../img/the_heroes.jpg" class="img-wods  img-thumbnail" alt="...">
                    <div class="overlay-text">
                        <div class="titulo-seccion">
                            The heroes
                        </div>
                    </div>
                </a>
            </div>
            <div class="wods col-md-6 col-lg-4 mb-4">
                <a href="desafios.php" class="image-container">
                    <img src="../img/desafios.jpg" class="img-wods  img-thumbnail" alt="...">
                    <div class="overlay-text">
                        <div class="titulo-seccion">
                            Desafios
                        </div>
                    </div>
                </a>
            </div>
            <div class="wods col-md-6 col-lg-4 mb-4">
                <a href="distancias.php" class="image-container">
                    <img src="../img/distancias.jpg" class="img-wods  img-thumbnail" alt="...">
                    <div class="overlay-text">
                        <div class="titulo-seccion">
                            Distancias
                        </div>
                    </div>
                </a>
            </div>
            <div class="wods col-md-6 col-lg-4 mb-4">
                <a href="qualifiers.php" class="image-container">
                    <img src="../img/qualifiers.jpg" class="img-wods  img-thumbnail" alt="...">
                    <div class="overlay-text">
                        <div class="titulo-seccion">
                            Qualifiers
                        </div>
                    </div>
                </a>
            </div>
            <div class="wods col-md-6 col-lg-4 mb-4">
                <a href="programas.php" class="image-container">
                    <img src="../img/programas.jpg" class="img-wods  img-thumbnail" alt="...">
                    <div class="overlay-text">
                        <div class="titulo-seccion">
                            Programs
                        </div>
                    </div>
                </a>
            </div>
        </div>
    </div>
</section>


<?php
include '../modulos/footer.php';
?>